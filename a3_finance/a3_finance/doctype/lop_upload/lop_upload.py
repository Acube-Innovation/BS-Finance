...
class LOPUpload(Document):
    ...
    def validate(self):
        ...
        # For Lop Per Request summary creation
        lop_days_summary = {}
        ...
        for idx, row in df.iterrows():
            try:
                ...
                # Single entry per row (no daily splits)
                self.append("lop_entries", {
                    "sl_no": sl_no,
                    "ec": ec,
                    "employee_name": name,
                    ...
                    "tl_hours": tl_hours,
                    "is_half_day": 1 if (no_of_days - int(no_of_days)) == 0.5 else 0
                })

                # Collect for Lop Per Request summary
                key = (ec, start_date)
        ...
        # Create Lop Per Request documents
        for (emp_id, start_date), data in lop_days_summary.items():
            frappe.get_doc({
                "doctype": "Lop Per Request",
                "employee_id": emp_id,
