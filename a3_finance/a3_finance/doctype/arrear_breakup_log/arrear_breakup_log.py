...
class ArrearBreakupLog(Document):
    ...
    def calculate_arrears(self):
        ...
        for slip_data in salary_slips:
            ...
            if lops:
                for lop in lops:
                    ...
                    new_bp_loss = new_bp_loss + (expected_bp * (flt(lop.no__of_days) / 30.0))
                    actual_sw_loss += flt(lop.employee_service_weightage_loss)
                    actual_da_loss += flt(lop.employee_da_loss_for_payroll_period)
                    current_da = ((expected_bp + actual_sw) * lop.employee_da_for_payroll_period)/30
        ...
        self.net_pay = round_half_up(flt(total_earnings - total_deductions))
