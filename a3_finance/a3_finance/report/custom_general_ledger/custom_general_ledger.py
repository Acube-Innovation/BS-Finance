...
def execute(filters=None):
	...
	if filters.get("party_1"):
		filters.party_1 = frappe.parse_json(filters.get("party_1"))

	if filters.get("party_2"):
		filters.party_2 = frappe.parse_json(filters.get("party_2"))
...
def validate_party(filters):
	party_type_1, party_1 = filters.get("party_type_1"), filters.get("party_1")

	if party_1 and party_type_1:
		for d in party_1:
			if not frappe.db.exists(party_type_1, d):
				frappe.throw(_("Invalid {0}: {1}").format(party_type_1, d))

	party_type_2, party_2 = filters.get("party_type_2"), filters.get("party_2")

	if party_2 and party_type_2:
		for d in party_2:
			if not frappe.db.exists(party_type_2, d):
				frappe.throw(_("Invalid {0}: {1}").format(party_type_2, d))

def set_account_currency(filters):
	if filters.get("account") or (filters.get("party_1") and len(filters.party_1) == 1):
		...
		if filters.get("account"):
			...
			gle_currency = frappe.db.get_value(
				"GL Entry",
				{"party_type": filters.party_type_1, "party": filters.party_1[0], "company": filters.company},
				"account_currency",
			)
